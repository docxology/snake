...
Exhaustive, validated searches for dimensions 11-16 using multiple aggressive
search strategies with comprehensive logging and progress saving. Moved
verbatim from ``snake_in_box/scripts/crack_high_dimensions.py`` so that both
the serial and parallel front-door scripts share one library implementation
(thin-orchestrator contract, 2026-09 scripts audit).
...
def run_all_dimensions(
    ...
    """Run the comprehensive search over all target dimensions.

    Orchestrates :func:`search_dimension` for each target dimension, with
    progress saving, interrupt handling, and a summary report.

    Parameters
    ----------
    target_dimensions : Optional[List[int]]
        Dimensions to search; defaults to ``TARGET_DIMENSIONS``.
    memory_limit_gb : float
        Memory limit for searches
